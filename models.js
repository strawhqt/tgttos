...
export class Models {
  ...
  drawChicken(context, program_state, model_transform, dead, wing_angle) {

    const dead_transform = dead ? Mat4.rotation(Math.PI / 2, 0, 0,  1) : Mat4.identity();
    if (dead) wing_angle = 0;

    const body_transform = this.backAndForth(Mat4.scale(0.6, 0.6, 0.75), 0, 1, 0).times(dead_transform);
